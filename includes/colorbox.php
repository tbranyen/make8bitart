<div id="colorbox" class="draggy">

	<div class="controls">
		<img class="ui-dragger" src="assets/draggybits/dragger.png">	
	</div>
	
	<div id="color" class="inner clearfix">
		<h3>pick a color:</h3>
		<a class="button color primary red" title="red" /> </a>
		<a class="button color primary orange" title="orange" /> </a>
		<a class="button color primary yellow" title="yellow" /> </a>
		<a class="button color primary green" title="green" /> </a>
		<a class="button color primary blue" title="blue" /> </a>
		<a class="button color primary purple" title="purple" /> </a>
		<a class="button color primary black" title="black" /> </a>
		<a class="button color primary gray" title="gray" /> </a>
		<a class="button color primary white" title="white" /> </a>
		<a class="button color favorite" title="jenn's pick" /> </a>
		<a class="button color transparent" title="erase" /> </a>
		<a class="button color custom" title="custom" /> </a>
	</div>
	
	<div id="colorpicker">
		<h3>hex value:</h3>
		<p>#<input id="hex-color" type="text" value="000000" /> <a href="#" title="color dropper"><img id="color-dropper" src="assets/dropper.png" alt="dropper"></a></p>
		
		<h3>click to pick:</h3>
		<img id="8bitcolors" src="assets/customcolors.png" />
		<div class="color-demo"></div>
	</div>

</div>